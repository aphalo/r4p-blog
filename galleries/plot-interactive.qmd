...
subtitle: "Dynamic plots with 'ggiraph' and 'plotly'"
...
Plotly can convert a regular ggplot into an interactive plot that can be embedded in HTML output. The advantage is that one uses the usual layer functions, but control of output is rather limited. It is implemented as a parsing engine that recognizes only some 'ggplot2' layer functions. As can be seen in the example below this approach makes rendering math difficult as expressions or text labels marked to be parsed into expressions are displayed as unparsed text.

Several of the statistics from 'ggpmisc' can still be used but the formating is not as nice as when using R graphic devices.
