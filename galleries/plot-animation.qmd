...
incomatible with package's 'gganimate'.
:::
...
explanations about the annotations. Here we focus on how to animate similar
plots to those described there.
...
Pearson correlation. Using `group` aesthetic we create two discrete scenes.
...
Confidence interval for R. Using `colour` aesthetic we create two discrete scenes.
...
to use other estimates like *P*-value. Here we first show the data followed
by the correlation estimate as a label.
...
not always best. Here we use labels to the left of each equation and build
the plot one group at a time.
