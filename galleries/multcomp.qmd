...
::: callout-warning
The examples in this page make use of packages
'ggpmisc' (>= 0.5.4) and 'ggpp' (>= 0.5.4) available through CRAN. Earlier
versions lack the layer functions described in this page.
:::
...
are supported and the order of letters in letter labels is fixed. This stat has been tested only with `method = "lm"`, `method = "aov"` and `method = "rlm"`although several other model fit functions can be expected also to work.
