...
With two factors we get a two-way table, and with additional factors, aditional 2D tables. From this very simple table we can see that we have 50 observations per species. Had we had wrongly typed the species name in one cell we would get a different table.
...
```
We now know that there is a badly encoded value, and we can find in which row it is.

```{r}
which(bad_iris$Species == "acetosa")
```

We can also replace, if we are sure that this is a mistake. The code below will replace all instances of `"acetosa"` by `"setosa"`.
...
I am not showing an example, but when reading data from a text file (e.g., `.csv` file) if the decimal mark (`,` or `.`) is not recognized as such, numbers are read as text (`character` values) instead numbers (`numeric` values). It is enough that a single value in a whole column has the wrong decimal, for the whole column to be read as text. `summary()` can also help in such a case, as the summary for the afected column will not be numeric. 

::: callout-tip
`NA` is an abbreviation for _Not Available_, it is used to indicate missing data. Observations that should have been available but were lost or are missing because of an "accident". They propagate in arithmetic operations. One or more missing values make the result that would have been obtained had the value(s) been available, also not available or unknown.
...
::: callout-caution
The advice is not to use box plots with small data sets, say fewer than 10 to 15 observations per box plot (e.g., for each of species in the figure above). The shape of the boxes and whiskers is quite reasonable with 50 observations per species, with few observations the plots become less informative than showing all the observations.
:::

An enhancement to simple box plots is to show an approximate 95% confidence interval as a notch in the box plot.
...
library(lattice)
ggplot(barley, aes(x = year, y = yield)) +
  stat_summary(fun = median, geom = "col", width = 0.67) +
  expand_limits(y = 0) +
...
ggplot(barley, aes(x = year, y = yield, fill = year)) +
  stat_summary(fun = median, geom = "col", colour = "black", width = 0.67) +
  expand_limits(y = 0) +
  scale_fill_grey() +
  facet_grid(cols = vars(site)) +
  theme_bw() +
