...
code-fold: false
abstract: |
  The use of function acq_irrad_interactive() from package 'ooacquire' >= 0.5.4 is described in detail. It can be used to acquire irradiance and fluence spectra, and time series of spectra with OceanOptics spectrometers.
---
...
[A video](https://flic.kr/p/2rUQ3xr) demonstrates the use of the interface of function `acq_irrad_interactive()` of 'ooacquire' (>= 0.5.4).

{{< video ooacquire-default.mp4 >}}
...
Interactively, the protocol is selected at the prompt, and the choice confirmed.
```
Protocols; light, filter, dark (l/ld/lf/lfd-/dl/dfl): ld
Will use protocol 'light -> dark', o.k.? (-/n):
```
with the list of protocols offered possibly modified by an argument passed when calling `acq_irrad_interactive()` (see section **User defined protocols**). 
...
Only one name needs to be provided by the user for each measurement or sequence of repeats, the _base name_ of the objects and files that will be created. The name is provided at the prompt

```
Give a name to the spectrum: <name>
```
where `<name>` is a syntactically valid R name entered by the user.

The actual object names will be create by appending `.spct` and `.raw_mspct` and the file names by appending `.spct.Rda` and `.pdf` to the base name. As the base name needs to be a syntactically valid name in the R language, it is validated before use.

Starting from version 0.4.3, _sequentially numbered names_ are available. They are implemented by appending a three digits number to the user-provided base name and then used as a base name as described above. Sequential numbering is started when the user inputs a base name ending in `#`. When a different name ending in `#` is entered the counter is reset, and sequential numbering starts from `001` and continues as long as the user accepts the same base name, shown by default. Starting with version 0.4.4, entering a new name ending in `%`, behaves as `#` but does not reset the counter. If the new name does not end in `#` or `%`, it is used as is, sequential numbering disabled and the counter reset. In the case of repeated measurements, sequential numbering is enforced even if the user enters a base name not ending in `#` or `%`.

When sequential names are not enabled the prompt for a new name is `Give a name to the spectrum: ` as shown above and 
...
## Attributes values to set

If the active `interface.mode` name ends in `-attr` the prompt

```
New acquisition.
Current 'what.measured': <previous-value>.
Current 'comment': <previous-value>.
Replace 'what.measured' or 'comment', or go (w/c/g-): <user input>
```
where by entering `w` or `c` a new value for 'what.measured' or 'comment' attributes, respectively, can be entered, `g` or <enter> advances to the next prompt and the current values are added to the spectral data objects as attributes. The 'how.measured', 'instr.desc', and 'inst.settings' attributes are always set automatically.

## Acquisition parameters

We describe the data acquisition parameters affecting the acquisition of spectral irradiance only briefly here, and refer the reader to the [description of the algorithms](https://www.r4photobiology.info/pages/ooacquire-algorithms.html).
...
Above the prompt the current value of each setting is shown. The prompt varies, as depending on the `interface.mode` in use some options are hidden. The integration time is labelled `integ(OK)` when the value has been already set or tuned, and labelled `integ(??)` when the tuning would need to be repeated or the value re-entered. In addition, depending on this and on the `interface.mode`, the default selected by typing `<enter>` varies, between `t` and `g`. In version 0.4.4, `MEASURE` was replaced by `GO` to make the text shorter. The full prompt is:

```
Acq: integ(??) = 0.01S, saturation = 0.05, tot. range = 7-10S, HDR mult. = 1,10
fixed/RETUNE/tune/sat/range/HDR/undo/help/go (f/t-/T/s/r/h/u/?/g):
```
With `t` or `<enter>` the next prompt is displayed.
```
Auto-adjust integration time? ADJUST/current/abort (a-/c/z): 
```
Entering `a` or `<enter>` starts the search, in this example converged after two attempts. Progress is shown, the final setting and the menu is displayed again.

```
Trying integration time (s):  0.01 
Max counts [0]: 2767.69 < 64558 target
Trying integration time (s):  1.088166 
Max counts [1]: 59702.34 < 64558 target
Trying integration time (s):  1.203579 
Max counts [2]: 66226.14 >= 64558 target

TUNED SETTINGS:
Relative saturation:         0.968 
Integration times (s):         1.2        7.2 
Numbers of scans:                 6          1 
Total time (s):                7.22       7.20 
Acq: integ(OK) = 1.2S, saturation = 0.05, tot. range = 7-10S, HDR mult. = 1,10
fixed/retune/tune/sat/range/HDR/undo/help/GO (f/t/T/s/r/h/u/?/g-): 
```
...
In the case of spectral fluence, although available, several of the corrections are in practice difficult to implement because of the discontinuous light emission by the sources usually measured. If pulses can be triggered on demand and the output per pulse is reproducible, stray light correction is applicable. Adjustment of signal level and eventually bracketing need to be done based on the number of light pulses or flashes per detector integration.
...
```
Duration of each repeated measurement: 29.7 s.
Seq: wait = 0S, boundary = none, step-len = 0S, step-reps = 1 
wait/boundary/step-len/step-reps/undo/help/GO (w/b/s/r/u/?/g-):
```

The character string used to set time values must be one interpretable as a `period` by function `lubridate::period()`. When a **period** is expected, bare numbers are not accepted, while `30S`, `5M`, and `1H`, as well `1H30M` are all accepted as periods. In the case of `boundary`, `none` is equivalent to `0S`.
