...
Frequently, the distinction between small, medium and big data is data un the number of numeric values a data set contains. This can be usefull from a computaional perspective, but not from a data analysis perspective. I use here a different criterion, the number of significance tests or contrasts relative to the number of independent replicates.
...
When we set a null hypothesis, in principle we can set it to any value instead of zero. In other words test for significance against a minimum size of response that is of interest. This is rarely done in practice, except for testing if a slope differs from one.
...
## Further reading
  ...
  - The booklet _The Dog xxxx _ proposes a middle ground in the philosophy of science controversy, as applied in Ecology. Mario Bunge has 
