...
The examples in this section follow the protocols only approximately. Some groups of computations are carried out together within a single function and here mimicked using simplified code for step by step computations. The examples also skip the consistency checks (e.g., spectrometer serial number and match of column names). In addition, the examples below were written some years back and need to be updated. _For these reasons, the computed spectral irradiances may differ slightly from those returned by in real use._ **An update is planned, using better example data.** 
